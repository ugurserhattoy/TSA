...
def test_add_and_get_application(model):
    # Add application
    model.add_application("Org1", "City1", "RoleA", "2024-07-01", "ContactA", "NoteA")
...
def test_update_application(model):
    model.add_application("Org2", "City2", "RoleB", "2024-07-02", "ContactB", "NoteB")
    app = model.get_applications_by_organisation("Org2", "City2")[0]
    app_id = app[0]
    model.update_application(app_id, "RoleC", "2024-07-03", "ContactC", "NoteC")
...
def test_delete_application(model):
    model.add_application("Org3", "City3", "RoleX", "2024-07-04", "ContactX", "NoteX")
...
def test_has_application(model):
    model.add_application("Org4", "City4", "RoleY", "2024-07-05", "ContactY", "NoteY")
...
def test_get_application_org_city_pairs(model):
    ...
    for org, city in pairs:
        model.add_application(org, city, "Role", "2024-07-06", "Contact", "Note")
...
def test_multiple_applications_same_org_city(model):
    ...
    model.add_application("OrgD", "CityD", "Role1", "2024-07-07", "Contact1", "Note1")
    model.add_application("OrgD", "CityD", "Role2", "2024-07-08", "Contact2", "Note2")
