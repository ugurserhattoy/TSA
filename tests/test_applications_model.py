...
def test_update_application(model):
    ...
    app_id = app[0]
    model.update_application(
        app_id, role="RoleC", date="2024-07-03", contact="ContactC", note="NoteC"
    )
...
def test_get_application_org_city_pairs(model):
    ...
    for org, city in pairs:
        model.add_application(
            org, city, role="Role", date="2024-07-06", contact="Contact", note="Note"
        )
